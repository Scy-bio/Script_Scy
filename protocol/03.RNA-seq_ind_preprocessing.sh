...
MARKDUP_JAVA_OPTIONS="-Xmx24g -XX:ParallelGCThreads=20  -Djava.io.tmpdir=${TEMP_GATK_DIR}" # Example: 8GB RAM, 20 threads for MarkDuplicates
SPLITNCIGAR_JAVA_OPTIONS="-Xmx24g -XX:ParallelGCThreads=10  -Djava.io.tmpdir=${TEMP_GATK_DIR}" # Example: 4GB RAM, 10 threads for SplitNCigarReads
HAPLOTYPECALLER_JAVA_OPTIONS="-Xmx24g  -Djava.io.tmpdir=${TEMP_GATK_DIR}" # Example: 8GB RAM for HaplotypeCaller

# --- Output Directory Structure ---
# All generated output will be stored within this base directory.
GATK_OUTPUT_BASE="test_result/03.ind_rnaseq_results"
...
# --- Usage Function ---
usage() {
  echo "Usage: $0 --genome <path/to/region.fa> --rnaseq <path/to/list_file>"
  echo ""
  echo "Options:"
  ...
  echo ""
  echo "Example:"
  echo "  ./03.RNA-seq_ind_preprocessing.sh --genome data/region.fa --rnaseq list_of_aligned_bams.txt"
  exit 1
}

# --- Parse Command Line Arguments ---
OPTIONS=$(getopt -o "" -l genome:,rnaseq: -- "$@")
...
echo "  Creating sequence dictionary for ${GENOME_FASTA}..."
echo "  Indexing reference genome with samtools faidx..."
echo "Reference genome prepared for GATK."
echo ""

# --- 3. Process BAM files for allele expression analysis ---
...
echo "You can now proceed with allele expression analysis"
echo ""
